...
class GameAPI {
    ...
    async registerOrLogin(googleProfile) {
        try {
            ...
            console.error('registerOrLogin error', e);
    ...
    async getOnlineStats() {
        try {
            ...
            const [recentLogs, todayLogs] = await Promise.all([
                this._fetch(`online_logs?created_at=gte.${encodeURIComponent(fifteenMinAgo)}&action=eq.login&select=user_uid`),
                this._fetch(`online_logs?created_at=gte.${encodeURIComponent(todayStart)}&action=eq.login&select=user_uid`)
            ]);
            ...
            return { online_now: 0, online_today: 0 };
        }
    }
    ...
    async addDiamonds(user_uid, amount, adminName) {
        try {
            ...
            return { success: false, error: 'Ошибка начисления' };
        }
    }
    ...
    async removeDiamonds(user_uid, amount, adminName) {
        try {
            ...
            return { success: false, error: 'Ошибка списания' };
        }
    }
    ...
    async saveSupportChats(chats) {
        try {
            ...
            console.error('saveSupportChats error', e);
            return false;
        }
    }
