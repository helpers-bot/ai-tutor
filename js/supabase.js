...
export const supabase = {
    ...
    getApprovedFeed() { return request('content?select=*,users(username,avatar_url)&order=created_at.desc'); },
    
    async createContent(d) {
        // Сохраняем в основную таблицу
        const data = await request('content', { method: 'POST', body: JSON.stringify({...d, status: 'pending'}) });
        // Добавляем в модерацию
        const user = this.getUser();
        await request('content_moderation', {
            method: 'POST',
            body: JSON.stringify({
                content_id: data[0]?.id || d.id,
                user_id: d.user_id,
