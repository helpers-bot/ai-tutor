            ...
            <div class="video-overlay">
                ...
                ${!canAccess ? `<div class="premium-lock" data-id="${item.id}" data-price="${item.price_stars}">
                    <div class="lock-content">
                        ...
                        <p>Пользователь @${item.users?.username||'user'} выставил контент за <b style="color:gold;font-size:24px">${item.price_stars} ⭐</b></p>
                        ${item.description ? `<p style="color:#ccc;margin-top:10px">📝 ${item.description.substring(0, 100)}${item.description.length>100?'...':''}</p>` : ''}
                        <button class="unlock-btn">Открыть за ${item.price_stars} ⭐</button>
                    </div>
                </div>` : ''}
            </div>
        </div>
...
function showUpload() {
    appEl.innerHTML = `<div class="page-container"><div class="upload-container">
        ...
        <div class="upload-form">
            ...
            <div id="premiumHint" style="display:none;background:#1a1a1a;padding:15px;border-radius:10px;color:gold;font-size:14px;margin-top:10px">💡 Чтобы ваш контент купили, максимально подробно опишите его в описании и поставьте 5 хештегов.</div>
    ...
    document.getElementById('isPremium').onchange = (e) => {
